...
def make_anomalies_for_ticker(ticker: str, min_dte: int, max_dte: int, target_dte: int) -> List[ParityAnomaly]:
    ...
    for _, row in ch.iterrows():
        ...
        score = abs(res) + 0.5 * abs(iv_gap) * spot * 0.01
        mny = K / spot if spot else np.nan
        side = "K>S (OTM Call side)" if K > spot else "K<S (OTM Put side)"
        anomalies.append(ParityAnomaly(
            ticker=ticker, asof=today, expiry=expiry, dte=dte, strike=K, spot=spot,
            side=side, moneyness=mny,
...
def format_report(anoms: List[ParityAnomaly], top_n: int = 5) -> str:
    ...
    rows = []
    hdr = "ticker asof expiry dte strike spot side mny call_mid put_mid call_iv put_iv parity_res iv_gap score".split()
    rows.append("\t".join(hdr))
    for a in anoms[:top_n]:
        rows.append("\t".join([
            a.ticker, str(a.asof), str(a.expiry), str(a.dte),
            f"{a.strike:.2f}", f"{a.spot:.2f}", a.side, f"{a.moneyness:.3f}",
            f"{a.call_mid:.2f}", f"{a.put_mid:.2f}",
            f"{a.call_iv*100:.2f}%", f"{a.put_iv*100:.2f}%",
            f"{a.parity_residual:.3f}", f"{a.iv_gap*100:.2f}%", f"{a.score:.3f}"
        ]))
    return "\n".join(rows)

def split_groups(anoms: List[ParityAnomaly]) -> Tuple[List[ParityAnomaly], List[ParityAnomaly]]:
    above = [a for a in anoms if a.strike > a.spot]
    below = [a for a in anoms if a.strike <= a.spot]
    return above, below
...
def main():
    ap = argparse.ArgumentParser(description="Put–Call Parity anomaly scanner (with K>S / K<S split)")
    ...
    ap.add_argument("--separate_reports", action="store_true", help="Print K>S and K<S groups separately")
    ap.add_argument("--only_above", action="store_true", help="Show only K>S (OTM Call side)")
    ap.add_argument("--only_below", action="store_true", help="Show only K<S (OTM Put side)")
    args = ap.parse_args()
    ...
    for t in tickers:
        ...
        if args.separate_reports or args.only_above or args.only_below:
            above, below = split_groups(anoms)
            if not args.only_below:
                print("-- K>S (OTM Call side) --")
                print(format_report(above, top_n=args.top_n))
                print()
                post_slack(f"{header}\n*K>S (OTM Call side)*\n```{format_report(above, top_n=args.top_n)}```")
            if not args.only_above:
                print("-- K<S (OTM Put side) --")
                print(format_report(below, top_n=args.top_n))
                print()
                post_slack(f"{header}\n*K<S (OTM Put side)*\n```{format_report(below, top_n=args.top_n)}```")
            use = above + below
        else:
        ...
        for a in use:
            all_rows.append({
                ...
                "strike": a.strike, "spot": a.spot, "side": a.side, "moneyness": a.moneyness,
                "call_mid": a.call_mid, "put_mid": a.put_mid,
                "call_iv": a.call_iv, "put_iv": a.put_iv,
                "parity_residual": a.parity_residual, "iv_gap": a.iv_gap, "score": a.score
            })
    if not all_rows:
    ...
    # If user asked for separate reports, write two files (K>S and K<S)
    if args.separate_reports:
        above_df = df[df["strike"] > df["spot"]]
        below_df = df[df["strike"] <= df["spot"]]
        saved = []
        if not args.only_below:
            out_above = f"parity_anomalies_KgtS_{today_str}.csv"
            above_df.to_csv(out_above, index=False)
            saved.append((out_above, len(above_df)))
        if not args.only_above:
            out_below = f"parity_anomalies_KltS_{today_str}.csv"
            below_df.to_csv(out_below, index=False)
            saved.append((out_below, len(below_df)))
        for out_name, cnt in saved:
            print(f"Saved: {out_name} ({cnt} rows)")
    else:
        out = f"parity_anomalies_split_{today_str}.csv"
